...
On [ability](special-vocabulary.md#ability) compounds (`ABIL + x + {a|u}`), **-l / -m / -n / -r** are **changeability** on both **`xa`** (capable) and **`xu`** (incapable) — same senses as unmet **`xu`**, not contact / standing / force. **`xe`** / **`xo`** are undefined on ability. Details: [special-vocabulary.md](special-vocabulary.md).
