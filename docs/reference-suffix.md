...
* **Linkers** (*but*, *however*, *therefore*) — Stay **-l** / **-m**; not names. Phrase-level *and* / *or* / *nor* / *and/or* / priority use `/z/` `/d/` `/b/` `/ɡ/` `/w/` `/h/` (`zam`, `zan`, `gam`, `gan`, `zel`, `zen`, …) with named **-n** where allowed (left fence). VP-level uses `/v/` (`vam`, `van`, `var`, `vel`, …) with soft **-n** and continue **-r** (like clauses) — see [coordination.md](coordination.md).
