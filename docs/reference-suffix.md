...
* **Linkers** (*but*, *however*, *therefore*) — Stay **-l** / **-m**; not names. Phrase-level *and* / *or* / *nor* / *and/or* / ranked conjunction use `/z/` `/d/` `/b/` `/ɡ/` `/w/` `/h/` (`zam`, `zan`, `zar`, `zor`, `zer`, `zur`, `gam`, `gan`, `zel`, `zael`, `zoel`, `zen`, …) with named **-n** where allowed and phrase **-r** on **a**/**o**/**e**/**u** for *something*/*anything*/*whatever-by-rank*/*some objection* (fence: left preferred, right close allowed). VP-level uses `/v/` (`vam`, `van`, `var`, `vel`, `vael`, `voel`, …) with soft **-n** and continue **-r** (like clauses) — see [coordination.md](coordination.md).
* **Clause coordinators** (`xa` / `xo` / `xu` / `xao` / `xe` / `xae` / `xoe` + ending) — Vowel = join type (**e** / **ae** / **oe** = ranked conjunction — unmarked / co-satisfiable / exclusive); **-l** / **-m** = closed vs open committed lists on every join vowel; **-n** = soft packaging (*and then* / uncertain reconstruction / soft ranking; **not** phrase named-list **-n**); **-r** continues that list (required before every non-initial conjunct; a second **-l**/**-m**/**-n** nests — e.g. `xol A xol B xor C` = *A or (B or C)*). Same left-open opener/`-r` pattern as [VP-level](coordination.md#vp-level-coordination). Full table: [coordination.md](coordination.md#opener-r-coordination).
