...
* **Quote / aside / mention / opaque markers** — Closed span series ([quotations.md](quotations.md)): **open** `{PoS}{TYPE}xa{ENDING}` (e.g. `daxal`); TYPE = **a** quote / **e** aside / **o** mention / **u** opaque; **-l** = exact; **-m** = paraphrase; no **-n** on opens. **Close** = **`xuxul`** (pop one) / **`xuxum`** (pop all) — not PoS/TYPE-matched; not clause-join soft **-n**. **-r** reserved for resume-prior-span on opens. Named material *inside* quote / mention / aside still takes ordinary PoS + **-n** (`z-Samn`, …). Opaque interiors are not parsed as Clarity.
...
Foreign / code / raw non-lexicon surfaces use [opaque spans](quotations.md) (`d|sushi|` / `duxal` … `xuxul`). Use–mention and titles use [mention](quotations.md). Content-word **-n** marks a **name or fixed label** on ordinary or mention-interior words. Span opens take only **-l** / **-m** (exact / paraphrase). **Nativized loans** (adapted Clarity root + ordinary PoS + ending) need no span — see [quotations.md § Mentions, opaque, and loan words](quotations.md#loans).
