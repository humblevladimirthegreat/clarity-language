...
* **Linkers** (*but*, *however*, *therefore*) — Stay **-l** / **-m**; not names. Word-level *and* / *or* / *nor* / *except* / *instead* are bare particles with **-l** / **-m** / **-n** (named lists); phrase-level uses `/z/` `/d/` `/b/` `/ɡ/` (`zam`, `zan`, `gam`, `gan`, …) — Oxford named **-n**. VP-level uses `/v/` (`vam`, `van`, `var`, …) with soft **-n** and continue **-r** (like clauses) — see [coordination.md](coordination.md).
