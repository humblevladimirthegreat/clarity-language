...
On [ability](special-vocabulary.md#ability) compounds — **host + `x` + {a\|u}** (primary: `v<sing>xun`) or fallback **`ABIL + x + {a\|u}`** — **-l / -m / -n / -r** are **changeability** on both **`xa`** (capable) and **`xu`** (incapable) — same senses as unmet **`xu`**, not contact / standing / force. **`xe`** / **`xo`** are undefined on ability. Need-root + `x…` stays [values](values.md). Details: [special-vocabulary.md](special-vocabulary.md).
