...
* **Linkers** (*but*, *however*, *therefore*) — Stay **-l** / **-m**; not names. Phrase-level *and* / *or* / *nor* / *and/or* / ranked conjunction use `/z/` `/d/` `/b/` `/ɡ/` `/w/` `/h/` (`zam`, `zan`, `gam`, `gan`, `zel`, `zael`, `zoel`, `zen`, …) with named **-n** where allowed (fence: left preferred, right close allowed). VP-level uses `/v/` (`vam`, `van`, `var`, `vel`, `vael`, `voel`, …) with soft **-n** and continue **-r** (like clauses) — see [coordination.md](coordination.md).
