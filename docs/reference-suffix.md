...
* **Quote / mention / aside markers** — Closed span series ([quotations.md](quotations.md)): shape `{PoS}{TYPE}x{EDGE}{ENDING}` (e.g. `daxal`); TYPE = **a** quote / **e** aside / **o** mention; EDGE = **a** open / **u** close; **-l** = exact; **-m** = paraphrase; no **-n** on these markers (proverbs → quote; formula-as-label → mention). **-r** reserved for resume-prior-span. Named material *inside* the brackets still takes ordinary PoS + **-n** (`z-Samn`, …). Not clause-join soft **-n**.
...
Foreign words, slang, and other non-lexicon surface forms use [mention spans](quotations.md) when the surface stays foreign (`d{sushi}` / `doxal` …). Content-word **-n** marks that you are treating material as a **name or fixed label**. Span markers themselves take only **-l** / **-m** (exact / paraphrase). **Nativized loans** (adapted Clarity root + ordinary PoS + ending) need no span — see [quotations.md § Mentions vs loan words](quotations.md#loans).
