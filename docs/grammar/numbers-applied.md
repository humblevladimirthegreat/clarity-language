...
| **When-frame** | Ordinary lexicon `/h/` (*until*, *before*, recency), clock/date above, or a closed mood whose **job** locates time ([LIVE](commentary.md#live-vs-memory) / [WITNESSED](commentary.md#live-vs-memory) / [PREDICT](plan-decision.md#predict)). [RESIDUE](commentary.md#residue) / [FORMER](commentary.md#former-climate) do **not** locate time. There is no past/future ending. A numeric payload (*3 days* on a unit) is relation + scalar + unit. |
...
3. A **SHARED continuum** `/ɡ/` immediately after the join (a lexicon line, or stock **`gurulel`** when the line is pure numeric or already clear).
...
Clock / date ranges: SHARED continuum **`gumel`** (or a host relation + `/b/`) with digit-string endpoints (`bal gumel b_15,00 b_16,00`). Circumstance **`hal`** is a [restrictor](restrictors.md).
...
| **`zen z+5`** | named unspecified extremum | *under-fives*-style label |
| **`zaen z+5`** | named equal-to band | *equal-to-5* / tie label |
| **`zuen z+5`** | named floor band | conventional *greater-than* label |

Same under `/d/` `/b/` `/ɡ/` (`gel g+5` = modifier *\<5*; `duel d+10` = object *\>10*). **`ae`** / **`oe`** single-item with a number are ordinary ranked single-item / triage. Boolean single-item (**`zal`** / **`zol`** / …) is *just this value*. Single-item **`zel z+`** (zero-group) is ordinary single-item on plural/unspecified amount; +∞ is **`z+e`** / **`g+e`**.
