...
Forms stay **reserved** under `/h/` / `/w/` (same spellings as other phrase slots), but only a **core set** has defined circumstance readings. `/w/` matches `/h/` with host = the adjective (`wal` / `wam` / `wual` / `wuam` / `war` / `wor` / `wur` and the non-bare core below).
...
| **`hol` / `hom`** | *either when A or when B (not both)* — exclusive trigger; **`hom`** also allows *or never* | — undefined |
...
- Beyond **`wal` / `wam` / `wual` / `wuam` / `war` / `wor` / `wur`** and the non-bare core, further `/w/` bare forms are undefined
