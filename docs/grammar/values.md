...
**Not the same job as:** prescription **`xe`** (*this act ought* to serve or protect this need). **`xo`** describes *doing for this need*. On `/ɡ/`, the same stance is the noun’s purpose (*a gift for relatedness*). *Walks so that Ululon sits* is an intended **event** ([so that](core.md#so-that), **`holalam`**), not a need. *A book for a hammer* as a swap is [exchange](core.md#exchange) (`huhanem`). *Tells on behalf of Ululon* is [proxy](core.md#proxy) (`hudagam`).
