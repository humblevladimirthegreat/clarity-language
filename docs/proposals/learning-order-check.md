...
A **home section** is the anchor's heading together with its subsections. "Taught at home" means some span in that subtree traces the construction. Text above a page's first band heading (the page intro) is outside every band.
...
1. **Order and report.** *Done.* [learning-order.ts](../../src/lint/learning-order.ts) builds section positions from the sidebar, the level-2 band headings, and heading order. The docs lint records the section of every traced span. It prints a one-line summary on every build; `npm run lint:agalan -- --order-report` lists every finding: forward uses, forward links, constructions not taught in their home section, home anchors outside every band, and uses outside every band. Section-level coverage (the "not taught at home" finding) is report-only for now, because 44 constructions fail it. The page-level `checkConstructionCoverage` still fails the build. Section-level coverage becomes a failure in phase 7, along with the rest of the check.
2. **Tone marks.** Emit prosody tokens, add sentence-grammar placement, and add constructions per mark and scope. This also catches misplaced marks, which pass today. It is independent of the other phases.
3. **Overlays.** Add the `anchor` column to lexicon-overlays.csv. Add `overlay.<sense_form>.<pos>` to the trace, and test that every row has an anchor that resolves. Remove the `word.reading.*` kinds that overlays now cover.
4. **Split the rest.** Split numbers, values, joins, spans and role compounds, plus any other coarse ID the phase 1 report shows spanning sections. A coarse ID whose forms all fall in one section stays as it is.
