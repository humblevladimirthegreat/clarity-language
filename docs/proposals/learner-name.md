...
- A second slot for the listener (possible follow-on; see [Open questions](#open-questions)).
- Changing the house cast. Third-person people stay **`azawa`** / **`ulula`** / **`uhubu`**.

## What counts as a proper name

A valid learner name is the **-n** citation of a row in [`data/lexicon-published.csv`](../../data/lexicon-published.csv): `azawan`, `agalan`, `uguron`, …
...
- **Morph glosses:** the slot's gloss renders reactively. The default is `z-speaker` (the overlay gloss). A chosen name shows the English name (`z-Uguron`), per [glosses.md](../meta/glosses.md) names.
...
- **Suggest:** shows one random valid root as its **-n** citation, with both senses, e.g. `uguron`: *grin* / *delight*. Each has a play button (IPA / speech) and a short line: "as a name, it carries both pictures". The mnemonic appears when the row has one.
- **Another:** rerolls. Keep the last few for back-navigation, and do not repeat within a session.
- **Choose this name:** saves it. The helper then switches to "Your Agalan name is `uguron`" with **Change** and **Clear**.
...
- Should hand-picking from the lexicon also apply the random helper's deny list, or only warn?
- Should there be a listener slot (`YOU` → default **`edonen`**) for dialogues? It would only fit pages where the site plays the other speaker.
