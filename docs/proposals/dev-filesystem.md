...
As measured on 2026-09-26, `npm test` took ~30 s and the rest of `npm run build` took another ~45 s. With `eslint --cache` and a warm cache, eslint still took 10 s, which is plugin loading.
...
- Only a partial fix. Repo reads still go through 9p: the doc lints and vitepress scan `docs/`, and tsc reads `src/`. Expect a 2–3× speedup, not 10×.
...
- Close to native speed for everything. Expect the full build to drop from ~75 s to about 10–20 s.
