...
| **u** | `zur` / `dur` / `bur` / `gur` / `wur` / `hur` / **`vur`** / **`xur`** | ***some catch*** — existential unspecified member of a *nor* / denial frame (*something’s off* / *objection* is a common interpersonal reading) | nullary: some catch / something’s off; unary: some catch involving that kind; multi: catch among some of the listed |
...
| **`vur` / `xur`** | *some catch (re doing)* / *something’s off* | *some catch involving X-ing / claim X* | *What’s the catch?* / *What’s wrong?* |

**Arity** (same length rule as other unary / nullary phrase forms):

| Length | **`…ar` (*something*)** | **`…or` (*anything*)** | **`…er` (*whatever-by-rank*)** | **`…ur` (*some catch*)** |
|--------|-------------------------|------------------------|--------------------------------|------------------------------|
| **0** (nullary) | *something* / *someone* | *anything* / *anyone* | *whatever’s most important* / *whatever comes first* | *some catch* / *something’s off* / *some no-go* |
| **1** (unary) | *some (unknown) X* / *an unspecified X* | *any X* | *X as a priority* / *preferentially some X* | *some catch involving X* / *not okay re X (unspecified how)* |
| **2+** (multi) | *some one among these* | *any of these* | *whichever of these ranks highest* / *the preferred one among these* | *catch among some of these* / *nor some (unspecified) of these* |

Mnemonic: **-r** = member not named; vowel = frame (**a** = inventory existence; **o** = exclusive menu; **e** = ranked preference; **u** = denial / *nor*). Positive twins of nullary emptiness: `zal` / `zam` → **`zar`**; `zol` / `zom` → **`zor`**; `zel` / `zem` → **`zer`** (*whatever matters*, vs *don’t care* / *I don’t have a ranking*); `zul` / `zum` → **`zur`** (*some catch*, vs *nothing ruled out* / *clear*).
...
- Revision **`ul` / `um`**: *except* a named right-hand side. Prefixed **`zur`**: unspecified denial in the slot (*some catch* / *something’s off*), not an *except*-revision particle.

**Under [polar question](language-reference.md#utterance-markers-j) force:** these forms are the **content-question** series (ask to name the unspecified member). Nullary `zar` / `zor` / `zer` / `zur` → *Who/what?* / *What (anything)?* / *What’s the priority?* / *What’s the catch?*; nullary `var` / `vor` / `ver` / `vur` → *What (did they) do?* / *Do anything?* / *What’s the priority (to do)?* / *What’s the catch (re doing)?*; nullary `xar` / `xor` / `xer` / `xur` → *What happened?* / *Anything?* / *What’s the priority (as outcome)?* / *What’s wrong?*; unary / multi parallel the phrase pattern in the queried slot. The answer **need not** keep **-r**: a fill may be a bare content word, unary **-l** / **-m** / **-n**, or a longer list with any allowed ending. Unbound *who* / *what* / *where* = nullary phrase **-r** in the queried slot (PoS = role); unbound *what did they do?* / *what happened?* = nullary **`var`** / **`xar`** (etc.) — no separate interrogative pronoun. Negation / NPI-flavored *anything* stays on `/h/` (or similar) when needed — `zor` / `vor` / `xor` themselves are free-choice, not negative-polarity markers.
...
- Nullary / unary / multi **…ar** / **…or** / **…er** / **…ur** — **content questions** (ask to name the unspecified member): unbound *who* / *what* / *where*, *what did they do?*, *what happened?*, *any X?*, *which of these?*, *what’s the priority?*, *what’s the catch?* / *what’s wrong?* — see [unspecified-member **-r**](#unspecified-member-r-phrase). Answer ending free (need not keep **-r**).

Unbound *who* / *what* / *where* = nullary **…ar** / **…or** / **…er** / **…ur** in the queried slot. **Length 2+** (no separate table): **-l** = pick from this closed menu (esp. **o** / **oe**); **-m** = pick, or name something else — answer ending likewise free; **…ar** / **…or** / **…er** / **…ur** = content Q among the listed (*which of these?* / *any of these?* / *whichever ranks highest?* / *which of these is sus?*).
...
| **…ur** (`zur` / `dur` / …) | *some catch involving X* / *not okay re X (unspecified how)* | Existential denial about kind X — not revision *except* (**ul** / **um**) | *What's sus about X?* / *What’s wrong with X?* (content Q) |
...
| **…ul** (`zul` / `dul` / …) | *nothing ruled out* / *clear* | Closed empty *nor*-list — denial set empty on purpose (also: *no objections*) | *Nothing ruled out?* / *Clear?* / *We’re good?* |
| **…um** (`zum` / `dum` / …) | *nothing I’m ruling out (yet)* | Open empty *nor* — withholding denial, not affirming “nothing exists” (also: *no objection from me yet*) | *Nothing you’re ruling out (yet)?* |
...
| **…ur** (`zur` / `dur` / …) | *some catch* / *something’s off* / *some no-go* | Positive twin of **…ul** / **…um** — unspecified denial (also: *some objection*) | *What’s the catch?* / *What’s wrong?* (content Q) |
...
- Unspecified-member: `zar` → *something*; `zar z-personl` / `z-personl zar` → *some (unknown) person*; `zar z-teal z-coffeel` → *some one among tea and coffee*; `zor` → *anything*; `zor z-personl` → *any person*; `zor z-teal z-coffeel` → *any of tea or coffee*; `zer` → *whatever’s most important*; `zer z-safetyl` → *safety as a priority*; `zer z-teal z-coffeel` → *whichever of tea or coffee ranks higher*; `zur` → *some catch*; `zur z-contractl` → *some catch involving the contract*; `zur z-costl z-riskl` → *catch among some of cost and risk*.
...
- Under polar Q: `var` → *What (did they) do?*; `vur` → *What’s the catch (re doing)?*
...
- `xar` → *something happened*; under polar Q → *What happened?*; `xur` → *some catch* / *something’s off*
