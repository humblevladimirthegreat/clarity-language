...
Content questions (*who* / *what* / *where* / *what did they do?* / *what happened?*) use [unspecified-member **-r**](coordination.md#unspecified-member-r-phrase) (`zar` / `zor` / `zer` / `zur` / `var` / `vor` / `ver` / `vur` / `xar` / …, and unary / multi) in the queried slot (PoS prefix = role) under the same **polar question** force as yes/no questions; there is no separate interrogative pronoun. Nullary **-m** under polar force confirms open emptiness (*Got nothing?*), not the fill-ask.

## Phrase-level, VP-level, and clause-level coordination

There is **no** bare word-level **coordinator** series. **`/z/` `/d/` `/b/` `/ɡ/` `/w/` `/h/` `/v/` `/x/`** + the coordination vowels + ending (`zam`, `dol`, `bem`, `gam`, `wam`, `ham`, `vam`, `xam`, `zar`, `zor`, `zer`, `zur`, `var`, `xar`, …) join same-slot phrases, VPs, or sentences with a **fence** (shared modifiers immediately after the coordinator — left preferred: `zam g-bigl z-dogl z-catl` = *big (dog and cat)*; right close allowed; nesting: left stack = right-assoc `vol A val B C` = *A or (B and C)*, right stack = left-assoc `A B vol C val` = *(A or B) and C*; **pure infix illegal**). On VP chains, `/h/` before a left fence scopes over all VPs; `/h/` inside a conjunct scopes that VP only. **-l**/**-m** = closed/open on every join vowel (**a** / **o** / **u** / **ao** / **e** / **ae** / **oe** / **uo** / **uao** / **ue** / **uae** / **uoe**); phrase **-n** = named; VP/clause soft **-n** = *and then…* / uncertain reconstruction / soft ranking; **-r** on **a**/**o**/**e**/**u** at all levels = [unspecified-member](coordination.md#unspecified-member-r-phrase) (`zar` / `zor` / `zer` / `zur` / `var` / `vor` / `ver` / `vur` / `xar` / … = *something* / *anything* / *whatever-by-rank* / *something weird* — VP *do something*, clause *something happened*; under polar Q = content questions); **e** / **ae** / **oe** are **ranked conjunction** (earlier ≻ later; **ae** = co-satisfiable, **oe** = exclusive, bare **e** = exclusivity unmarked; preference vs fallback from context); leading **u** on **o** / **ao** / **e** / **ae** / **oe** = [empty-allowed](coordination.md#empty-allowed-u-prefix) (`uo` = at most one / *or neither*; `uao` = zero or more; `ue` / `uae` / `uoe` = ranked with abstention / *else nothing* — not bare **u** *nor*; **-m** does not imply empty-allowed). Phrase unary / nullary: **…em** / **…el** / **…oem** / **…oel** / **…uol** / **…uom** / **…uoel** / **…en** / **…al** / **…am** / **…an** / **…ol** / **…om** / **…aol** / **…aom** / **…ul** / **…um** / **…ar** / **…or** / **…er** / **…ur** (unary: *X first* / *only X matters* / *try X first* / *only try X* / *X or nothing* / …; nullary: *don’t care* / *nothing comes to mind* / *null* / *no options* / *fine to pick none* / …; **…an** = *null* / *void*, **…en** = *it’s a draw*, **…ar** = *something*, **…or** = *anything*, **…er** = *whatever-by-rank*, **…ur** = *something weird* — [coordination.md](coordination.md#unary-phrase); VP/clause unary/nullary on **-r** only; under polar question: **…ar** / **…or** / **…er** / **…ur** = content questions, unary **-m** offer, nullary **-m** confirm open emptiness, nullary **-l** colloquial emptiness check). Correlatives come later.
...
Phrase-level *and* / *or* / *nor* / *and/or* / ranked / empty-allowed joins are **not** general `/x/` linkers — they use `/z/` `/d/` `/b/` `/ɡ/` `/w/` `/h/` + the coordination vowels + ending (`zam`, `gam`, `zar`, `zor`, `zer`, `zur`, `zel`, `zael`, `zoel`, `zuol`, `zuoel`, …) with a **fence** (left preferred; right close allowed — [coordination.md](coordination.md)). VP-level uses `/v/` (`vam`, `var`, `vel`, `vael`, `voel`, `vuol`, `vuoel`, …) — same fence; **-r** = unspecified action (*do something*). Clause-level forms **are** `/x/` + the same vowels + ending (`xal`, `xar`, `xel`, `xael`, `xoel`, `xuol`, …) — see [coordination.md](coordination.md#clause-level-coordination).
