...
Left-bound is optional style (meter, focus, L1 comfort). Prefer default postposed order in neutral prose. On [phrase fences](coordination.md#scope-fence-p-coord), ordinary `/ɡ/` in the SHARED slot under **`a`** **distributes** over conjuncts when gradable (`zal g-bigl z-Samn z-Lean` = *Sam and Lea are big*) or is whole-phrase description when non-scalar (`zam g-ofl b-Samn z-dogl z-catl`); under **`ae`**, SHARED scale `/ɡ/` is [equative](comparatives.md#equatives) (`zael g-bigl z-Samn z-Lean` = *as big as*); under invert **ua** / **uo**, SHARED `/ɡ/` is the [kind / domain](coordination.md#universals-domains-generics) (`zual g-catl` = *every cat*); under ranked **`e` / `oe` / `ue`**, SHARED `/ɡ/` is the [comparison scale](comparatives.md) (`zel g-bigl z-Samn z-Lean` = *Sam is bigger than Lea*; unary `zel g-bigl z-Samn` = *Sam is the biggest*; nullary `zoel g-bigl` = *there is no biggest*; nullary `zel g-bigl` = *unspecified who’s biggest*); under **`a` / `e` / `ue` / `ua`** with exactly two number endpoints, SHARED continuum `/ɡ/` is a [numeric span](numbers.md#ranges) (`zel g-agel z+3l z+5l` = *from age 3 to 5*; bare `zel z+3l z+5l` = preference). `gl-` after a left fence binds only the **next** conjunct (`zam gl-bigl z-dogl z-catl` = *(big dog) and (cat)*).
...
There is **no** bare word-level **coordinator** series. **`/z/` `/d/` `/b/` `/ɡ/` `/v/` `/x/`** + the coordination vowels + ending (`zam`, `dol`, `bem`, `gam`, `vam`, `xam`, `zar`, `zor`, `zer`, `zur`, `var`, `xar`, …) join same-slot phrases, VPs, or sentences with a **fence** (shared modifiers immediately after the coordinator — left preferred: `zam g-ofl b-Samn z-dogl z-catl` = *Sam’s (dog and cat)*; right close allowed; nesting: left stack = right-assoc `vol A val B C` = *A or (B and C)*, right stack = left-assoc `A B vol C val` = *(A or B) and C*; **pure infix illegal**). **`/h/`** and **`/w/`** use the **same fence forms** as [circumstance restrictors](coordination.md#circumstance-restriction-h-w) (defined core: `hal` = *never*, `hal h-rainl` = *only when raining*, `hual` = *always*, `har` = *sometime* (under question → *When?* — [questions.md](questions.md#circumstance-when)); sibling manner/adjunct *and* is **juxtaposition**; other `/h/`/`/w/` coordinator spellings reserved but undefined). On VP chains, `/h/` before a left fence scopes over all VPs; `/h/` inside a conjunct scopes that VP only. **-l**/**-m** = closed/open on every join vowel (**a** / **o** / **u** / **ao** / **e** / **ae** / **oe** / **ua** / **uo** / **ue**); phrase **-n** = named; VP/clause soft **-n** = *and then…* / uncertain reconstruction / soft ranking; **-r** on **a**/**o**/**e**/**u** at all levels = [unspecified-member](coordination.md#unspecified-member-r-phrase) (`zar` / `zor` / `zer` / `zur` / `var` / `vor` / `ver` / `vur` / `xar` / … = *something* / *anything* / *whatever-by-rank* / *something else* (other than) — VP *do something*, clause *something happened*; under [question](questions.md) = fill-ask; under `/h/` `/w/` = *sometime* / *anytime* / … — *When?* under question: [questions.md](questions.md#circumstance-when)); **e** / **oe** / **ue** are **directional ranked** conjunction (**e** / **oe**: earlier ≻ later; **oe** = exclusive; **ue** = rank reversal); **ae** = **equality / tie** (multi + SHARED scale = [equative](comparatives.md#equatives); nullary **ae** = draw). On NP fences, SHARED scale `/ɡ/` under **`e` / `oe` / `ue`** = [comparatives](comparatives.md) (multi = *bigger than*; unary **`e`** = **superlative** — `zel g-bigl z-Samn`; nullary **`oe`** = **empty superlative** — `zoel g-bigl`; nullary **`e`** + scale = unspecified top — `zel g-bigl`); under **`ae`** multi = *as … as* — `zael g-bigl z-Samn z-Lean`; under **`a`** = distributive — `zal g-bigl z-Samn z-Lean`; bare **u** = [negation](coordination.md#negation-u) (unary *not X*, multi *none of*, nullary *no*; no separate `/h/` “not” root); leading **u** on **a** / **o** / **e** = [invert](coordination.md#invert-u-stacks) (`ua` = *everything but*; `uo` = *anything but*; `ue` = **rank reversal**, later ≻ earlier — not bare **u** [negation](coordination.md#negation-u) (*not* / *none of* / *no*); no three-vowel stacks; empty-allowed / *or none* on bare **o** + **-m** only (`zom` / `vom` / `xom` = at most one; `zol` / `vol` / `xol` = exactly one — [coordination.md](coordination.md#empty-allowed-om)); **-m** on other join vowels does not imply *or none*). Phrase unary / nullary: **…em** / **…el** / **…ael** / **…aem** / **…aen** / **…oem** / **…oel** / **…uem** / **…uel** / **…en** / **…al** / **…am** / **…an** / **…ol** / **…om** / **…ual** / **…uam** / **…uol** / **…uom** / **…aol** / **…aom** / **…ul** / **…um** / **…un** / **…ar** / **…or** / **…er** / **…ur** (unary: *X first* / *only X matters* / *try X first* / *only try X* / *X last* / *not X* / *everything but X* / *anything but X* / …; nullary: *unspecified ranking* / *nothing comes to mind* / *null* / *no options* / *no* / *everything* / *anything (goes)* / …; **…an** = *null* / *void*, **…aen** = *it’s a draw*, **…en** = *unspecified ranking* (stock), **…ar** = *something*, **…or** = *anything*, **…er** = *whatever-by-rank*, **…ur** = *something else*, **…un** = named/conventional *not X* / *no* — [coordination.md](coordination.md#unary-phrase); under `/h/` `/w/`: `hal` = *never*, `hual` = *always*,  = *equally often*, … — [circumstance](coordination.md#circumstance-restriction-h-w); VP/clause unary/nullary on **-r** and on negation **…ul** / **…um** / soft **…un**; interrogative readings: [questions.md](questions.md)). Correlatives come later.
...
Numeral grammar (digit roots, [digitless / zero-group forms](numbers.md#zero-digit-groups), engineering/scientific forms, ordinals, digit-strings, `/x/` enumeration, number-word shape, [ranges](numbers.md#ranges) / [thresholds](numbers.md#numeric-thresholds) / [measure phrases](numbers.md#measure-phrases)) lives in **[numbers.md](numbers.md)**.

# Comparatives, superlatives, and equatives

Scalar comparison (ranked **`e` / `oe` / `ue`** *more / …-est*, **`ae`** *as … as*, distributive **`a`** + SHARED `/ɡ/`, measured differentials): **[comparatives.md](comparatives.md)**. Measure NP shape (unit + amount): **[numbers.md § Measure phrases](numbers.md#measure-phrases)**.

# Phrase-level, VP-level, and clause-level coordination

Phrase-level **zam** / **gam** / …, VP **vam** / **var** / …, and clause `/x/` + vowel + ending (soft **-n** ≠ named **-n**; **-r** = unspecified-member at all levels): **fence** (left preferred, right close for style/comedy; shared modifiers immediately after the coordinator; [fence nesting](coordination.md#fence-nesting)); SHARED scale `/ɡ/` = [comparatives / equatives](comparatives.md) (`zel g-bigl z-Samn z-Lean` / `zael g-bigl z-Samn z-Lean` / `zal g-bigl z-Samn z-Lean` = distributive *both big* / unary `zel g-bigl z-Samn` / nullary `zoel g-bigl` = *no biggest*); SHARED continuum `/ɡ/` + two number endpoints = [number ranges](numbers.md#ranges) (`zel g-agel z+3l z+5l` = *from age 3 to 5*; bare `zel z+3l z+5l` = preference); invert **ua** SHARED `/ɡ/` = [kind domain / generics](coordination.md#universals-domains-generics) (`zual g-catl` = *every cat*; `hual` = habitual *always*); prefix-less **al** / **am** / **an** / **el** / … (**-l** / **-m** / named **-n** required; parallel chains on A) = in-clause revision (*including* / *rather* / *instead* / *except*): **[coordination.md](coordination.md)**. Number ranges and measure phrases: **[numbers.md](numbers.md)**. No bare word-level coordinators.
