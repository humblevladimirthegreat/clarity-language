...
There is **no** bare word-level **coordinator** series. **`/z/` `/d/` `/b/` `/ɡ/` `/w/` `/h/`** + the coordination vowels + ending (`zam`, `dol`, `bem`, `gam`, `wam`, `ham`, …) join same-slot phrases or single-word siblings with a **fence** (shared modifiers immediately after the coordinator — left preferred: `zam g-bigl z-dogl z-catl` = *big (dog and cat)*; right close allowed: `z-dogl z-catl zam g-bigl`). **`/v/`** and **`/x/`** use shared **left-open opener / `-r` continue** (`vam A var B`, `xal A xar B`, …). On VP chains, `/h/` before the opener scopes over all VPs; `/h/` inside a conjunct scopes that VP only. Phrase **-l**/**-m**/**-n** = closed/open/named on every join vowel (**a** / **o** / **u** / **ao** / **e**); **e** is **priority ranking** (earlier ≻ later; preference vs fallback from context). Opener soft **-n** and continue **-r** replace named **-n**. Correlatives come later.
...
Phrase-level *and* / *or* / *nor* / *and/or* / priority ranking are **not** general `/x/` linkers — they use `/z/` `/d/` `/b/` `/ɡ/` `/w/` `/h/` + the coordination vowels + ending (`zam`, `gam`, `zel`, …) with a **fence** (left preferred; right close allowed — [coordination.md](coordination.md)). VP-level uses `/v/` (`vam`, `var`, `vel`, …) — left-open opener / **-r**. Clause-level forms **are** `/x/` + the same vowels + ending (`xal`, `xel`, `xem`, …) — see [coordination.md](coordination.md#clause-level-coordination).
