...
| `/x/` (`/ʒ/`) | discourse marker (sentence linkers, clause continue, …) |
...
Property and kind claims use **zero-copula**: subject + predicative `/ɡ/` (no general *to-be* verb).
...
It sits on its host: normally right after the `/ɡ/` (or after that `/ɡ/`’s contiguous `/b/` if complex), including when the `/ɡ/` is left-bound. Unlike `/h/`, `/w/` does not float. Several `/w/` words may juxtapose on one `/ɡ/` (`guzem wazebol wazebel`); that is ordinary co-stacking. If a relation needs an argument, use complex `/ɡ/` + `/b/` instead.
...
**Topic** (*as for X*, *regarding X*) is complex `/h/` + `/b/` topic noun. Highlight focus by rearranging free word order (and ordinary intensifiers if needed). Topic/focus are not `/x/`.
...
Epistemic hedges (*apparently*, *allegedly*, …) are `/h/` or `/w/` — discourse glue like *however* / *therefore* is `/x/` ([below](#discourse-markers-x)).
...
| Shape | Job | Example |
|-------|-----|---------|
| `/ɡ/` + `/b/` | complex adjective | `gogunol buzuzul` (*same as…*) |
...
3. Optional **discourse reviser** — prefix-less **`al` / `am` / `an` / `el` / …** (*additionally* / *in other words* / *instead* / *except*) immediately before force or before an omitted-default body
...
When force is written, it closes the opener and does not reappear in the body. A new `/j/` turn starts a new utterance. `/x/` continues under inherited force.
...
Period marks **body closure**. Commitment strength is **`jal`** vs **`jam`** (and polar **`-l`** vs **`-m`**); a new `/j/` cluster starts a new turn.
...
| Form | Force |
|------|--------|
| **jal** | **statement** — assertoric claim or description |
| **jam** | **hedged statement** — tentative / provisional assert |
| **jol** | **question** — yes/no and content / fill-ask |
| **jom** | **soft question** — wonder, gentle ask, offer-like question |
| **jel** | **imperative** — command / instruction |
| **jem** | **request** — soft directive (*please…*) |
| **jul** | **prohibition** — *don’t…* (closed) |
| **jum** | **soft prohibition** — *please don’t…* / *I’d rather you not…* |
...
Closed `/j/` particles such as **`jael`** (*yes* / *true*), **`juel`** (*no* / *false*), **`jaol`**, and soft **-m** twins mark stance as interjections — left-edge or bare utterance, not clause force. **`-l`** implies **`jal`**; **`-m`** implies **`jam`**. Prefer **`jael …`** over **`jael jal …`** when a body follows.
...
Word-initial **x** is this discourse PoS. Mid-word **x** joins roots inside a compound (`zuzuzuxogeven`) — a different job.
...
| Linker | Lexicon root | Example |
|--------|--------------|---------|
| *therefore* | **`amala`** | `xamalal` |
| *however* | **`ezabe`** | `xezabel` |
| *meanwhile* | **`ameno`** | `xamenol` |
| *next* | **`uvulu`** | `xuvulul` |
| *but* | **`onuzu`** | `xonuzul` |
...
**Adverbial** *because* / *if* / *iff* / … = `/h/` relation + `/b/` **`odo`** (contiguous, matrix-final), e.g. `hurugul bodo` …
