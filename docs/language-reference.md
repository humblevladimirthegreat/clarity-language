...
Prefixed `/w/` **joins** (`wal` / `wam` / …, `war` / …) are **not** sibling adjunct *and*. They are [restrictors](coordination.md#circumstance-restriction-h-w) on when the host adjective applies (`wal` → *never* for that property; `wal w-allegedl` → *only when alleged*). Same fence rules; `/w/` exact parallel of `/h/` (**defined core** only).
