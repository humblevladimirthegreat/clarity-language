...
## Predicative pattern (zero-copula)
<a id="zero-copula"></a>

A clause may assert a property of a subject **without** a `/v/`: utterance force (`/j/`) + subject NP + predicative `/ɡ/` (plus any `/w/` / contiguous `/b/` on that `/ɡ/`). Mood, tense, evidentiality, and values still attach with `/h/` (clause) or `/w/` (on the property).
...
**Identity** = two expressions pick **one** referent (*Clark is Superman*, *this is that*). Do **not** use bare multi **`ae`** (that is equal rank / tie) or **`ae`** + SHARED scale (that is scalar equative).
...
- Closed roots inventory pointer: [special-vocabulary.md](special-vocabulary.md)
