...
This page is the source of truth for **values** (needs): which psychological payoff a host serves or costs, how that stake is framed, and — on the ending — either **contact channel** (when the need is met) or **changeability** (when unmet). Port of Claritish value tags (`+a` / `-ct`, `my+c`, `should+r`, …) into native Clarity morphology. Restrictors: [restrictors.md](restrictors.md). Revisers (slot repair, not values): [revisers.md](revisers.md). Ordinary reference-suffix senses: [reference-suffix.md](reference-suffix.md).
...
4. **Ending** — on value compounds with an `x`-addition, sense depends on the stake vowel ([contact channel](#value-contact) vs [changeability](#value-changeability)), not ordinary literal / metaphorical / name / pronoun.

**Bare need** (no `x`-addition): names the need only — no satisfies / detracts claim (topic-only). Ending on a bare need keeps ordinary [reference-suffix](reference-suffix.md) senses for now.

**Compound need** (`…x{a|e|o|u}…`): judgment, gratitude, ought, and motive ascriptions.
...
Same vowel letter logic as [revisers](revisers.md) (*including* / *rather* / *instead* / *except*) and joins (**a** additive, **e** soft rank, **o** exclusive swap, **u** subtract). Do **not** swap this table with the ending tables.
...
**Which ending table?** **`xa` / `xe` / `xo`** → [contact channel](#value-contact). **`xu`** → [changeability](#value-changeability).

## Contact channel (endings on satisfies / rather / instead)
<a id="value-contact"></a>
<a id="value-savoring"></a>

On **`xa` / `xe` / `xo`** compounds, **-l / -m / -n / -r** mark **how you contact** the (claimed) payoff — savoring / gratitude channel. Not tense (use ordinary tense elsewhere). Not changeability.
...
| Form | Contact |
|------|---------|
| `…xal` / `…xel` / `…xol` | **physical** channel |
| `…xam` / `…xem` / `…xom` | **mental** channel |
| `…xan` / `…xen` / `…xon` | **social** channel |
| `…xar` / `…xer` / `…xor` | **spiritual** channel |

No ranked “better” channel — each is legitimate.

## Changeability (endings on detracts)
...
### Satisfies / rather / instead — contact channel
...
| Ought / normative | clause + `h-NEED` + **`xa` / `xe` / `xo`** + channel | `should+r` |
| Motive (ex-*need to* / *have to*) | clause + `h-NEEDxa…` + channel | `I+r am …` |

Prefer **`xa`** for gratitude / praise; **`xu`** for unmet; **-m** on **`xu`** when changeability is unclear. Bare need words are fine for topic-only mention.
...
| `… h-relatxem` | rather relatedness; **mental** contact with that stake |
...
- Do **not** put satisfies/detracts on endings, or contact/changeability on `x`-vowels.
...
- **`xa` / `xe` / `xo`** endings are **contact channel**; **`xu`** endings are **changeability** — same letters, different tables.
