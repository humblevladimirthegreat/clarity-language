...
c) Easy to tell the components of a compound word because **x** separates them. Mid-word **x** is only the compound joiner (never part of a root); word-initial **x** is the discourse-marker prefix. Ordinary compounds include lexicon sense-compounds, [values](values.md), [ability](special-vocabulary.md#ability), [role compounds](special-vocabulary.md#role-compounds) (**`a`/`u`/`o` x ROOT**), and compact **[phrasal proper names](reference-suffix.md#phrasal-proper-names)** (given+family, *New York* — one word, e.g. `z-MaryxSmithn`; not adjacent `z-Maryn z-Smithn`; long work titles prefer [cite](quotations.md) instead). Closed [span fences](quotations.md) also use mid-word **`x`**: **open** `{PoS}{TYPE}x{EDGE}{ENDING}` (TYPE **a** cite / **e** aside / **o** mention / **u** opaque; EDGE **a** multi / **e** clause-scoped / **o** atomic / **u** empty; e.g. `daxal`, `daxol`, `daxan`, `daxar`, `daxul`); **close** **`xuxul`** (pop one) / **`xuxum`** (pop all). Full mid-word **`x`** family table: **[x-compounds.md](x-compounds.md)** (span = vowel×vowel; role = vowel×root; values/ability = root×vowel; sense/name = root×root).
...
g) **Span fences (cite / aside / mention / opaque):** [Opens](quotations.md) are ordinary PoS + TYPE vowel (**a** / **e** / **o** / **u**) + mid-word **`x`** + EDGE vowel (**a** / **e** / **o** / **u**) + ending (**-l** exact / **-m** paraphrase / **-n** proper / **-r** anaphor). EDGE **`o`** = atomic (one token). **Closes** are only **`xuxul`** / **`xuxum`** (not clause joins `xul` / `xum`, not left-bound `l`, not a global `xl` cluster; not empty open `daxul`). No stacked vowels on TYPE or EDGE. Discourse-only opens use PoS `/x/` (`xaxal`, `xuxal`, …).