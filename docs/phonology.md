...
c) Easy to tell the components of a compound word because **x** separates them. Mid-word **x** is only the compound joiner (never part of a root); word-initial **x** is the discourse-marker prefix. Ordinary compounds include lexicon sense-compounds, [values](values.md), [ability](special-vocabulary.md#ability), and compact **[phrasal proper names](reference-suffix.md#phrasal-proper-names)** (given+family, *New York* — one word, e.g. `z-MaryxSmithn`; not adjacent `z-Maryn z-Smithn`; long work titles prefer [mention](quotations.md) instead). Closed [span fences](quotations.md) also use mid-word **`x`**: **open** `{PoS}{TYPE}x{EDGE}{ENDING}` (TYPE **a** quote / **e** aside / **o** mention / **u** opaque; EDGE **a** multi / **e** clause-scoped / **u** empty; escape **`xoxol`**; e.g. `daxal`, `daxan`, `daxar`, `daxul`); **close** **`xuxul`** (pop one) / **`xuxum`** (pop all). Parser cue: after PoS, a single TYPE vowel then `x` then a single EDGE vowel + ending → span form; exact `xuxul` / `xuxum` → span close; exact `xoxol` → escape next token; a longer root before `x` → ordinary compound.
