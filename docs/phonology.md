...
c) Easy to tell the components of a compound word because **x** separates them. Mid-word **x** is only the compound joiner (never part of a root); word-initial **x** is the discourse-marker prefix. Closed [span fences](quotations.md) also use mid-word **`x`**: **open** `{PoS}{TYPE}x{EDGE}{ENDING}` (TYPE **a** quote / **e** aside / **o** mention / **u** opaque; EDGE **a** multi / **e** clause-scoped / **o** soft / **u** empty; e.g. `daxal`, `daxan`, `daxar`, `daxul`); **close** **`xuxul`** (pop one) / **`xuxum`** (pop all). Parser cue: after PoS, a single TYPE vowel then `x` then a single EDGE vowel + ending → span form; exact `xuxul` / `xuxum` → span close; a longer root before `x` → ordinary compound (lexicon, [values](values.md), [ability](special-vocabulary.md#ability)).
...
g) **Span fences (quote / aside / mention / opaque):** [Opens](quotations.md) are ordinary PoS + TYPE vowel (**a** / **e** / **o** / **u**) + mid-word **`x`** + EDGE vowel (**a** / **e** / **o** / **u**) + ending (**-l** / **-m** / **-n** / **-r**). **Closes** are only **`xuxul`** / **`xuxum`** (not clause joins `xul` / `xum`, not left-bound `l`, not a global `xl` cluster; not empty open `daxul`). No stacked vowels on TYPE or EDGE. Discourse-only opens use PoS `/x/` (`xaxal`, `xuxal`, …).