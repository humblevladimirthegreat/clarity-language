...
e) **Coordination-particle exception:** Bare **al**, **am**, **an**, **ol**, **om**, **on**, **ul**, **um**, **un**, **aol**, **aom**, **aon**, **el**, and **em** are [word-level coordinators](coordination.md). They have **no** PoS prefix; an ending is required (**-l** / **-m** / **-n** on **a** / **o** / **u** / **ao** = closed / open / named; **-l** / **-m** on **e** = exception / replacement — not ordinary reference-suffix senses). [Phrase-level](coordination.md#np--and-ap-level-coordination) (`/z/` `/d/` `/b/` `/ɡ/`: `zal`, `zam`, `gal`, `gam`, …) and [VP-level](coordination.md#vp-level-coordination) (`/v/`: `val`, `vam`, `var`, …) coordinators are the same vowel series — ordinary PoS+root+ending shape, but those vowel roots are reserved as coordinators only. Word-level is **Oxford-style** (bare coordinator once, **before** the last conjunct; **no** `-r`). Phrase-level puts the prefixed coordinator **after** the last conjunct (scope fence; **no** `-r`). VP-level and clause-level use **opener / `-r` continue**. For bare particles, boundaries stay clear because the previous word ends in a reference suffix (or **-sh**) and the next word begins with a PoS consonant. These forms are reserved; they are not lexicon roots.
