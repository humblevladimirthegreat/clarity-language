...
**Writing:** these pieces are one unbroken token — **no hyphen** after the PoS letter (`zivon`, not `z-ivon`). Foreign roots use `PoS<…>ENDING` (e.g. `d<english>l`). Full orthography note: **[language-reference.md § Orthography](language-reference.md#orthography)**. Doc examples that show `z-…` are pedagogical boundary marks only.
