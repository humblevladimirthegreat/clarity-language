...
e) **Coordination-particle exception:** Bare **al**, **am**, **an**, **ol**, **om**, **on**, **ul**, **um**, **un**, **aol**, **aom**, **aon**, **el**, and **em** are [word-level coordinators](coordination.md). They have **no** PoS prefix; an ending is required (**-l** / **-m** / **-n** on **a** / **o** / **u** / **ao** = closed / open / named; **-l** / **-m** on **e** = exception / replacement — not ordinary reference-suffix senses). [NP-level](coordination.md#np-level-coordination) (`/z/` `/d/` `/b/`: `zal`, `zam`, …), [AP-level](coordination.md#ap-level-coordination) (`/ɡ/`: `gal`, `gam`, …), and [VP-level](coordination.md#vp-level-coordination) (`/v/`: `val`, `vam`, `var`, …) coordinators are the same vowel series — ordinary PoS+root+ending shape, but those vowel roots are reserved as coordinators only. Word-, NP-, and AP-level are **Oxford-style** (coordinator once, before the final conjunct; **no** `-r`). VP-level and clause-level use **opener / `-r` continue**. For bare particles, boundaries stay clear because the previous word ends in a reference suffix (or **-sh**) and the next word begins with a PoS consonant. These forms are reserved; they are not lexicon roots.
