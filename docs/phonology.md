...
e) **Coordination roots:** [Phrase-level](coordination.md#phrase-level-coordination) (`/z/` `/d/` `/b/` `/ɡ/` `/w/` `/h/`: `zal`, `zam`, `gal`, `gam`, `ham`, …), [VP-level](coordination.md#vp-level-coordination) (`/v/`: `val`, `vam`, `var`, …), and [clause-level](coordination.md#clause-level-coordination) (`/x/`: `xal`, `xam`, `xar`, …) coordinators use the same vowel series under an ordinary PoS+root+ending shape; those vowel roots are reserved as joiners only (**-l** / **-m** / **-n** on **a** / **o** / **u** / **ao** = closed / open / named at phrase level; soft **-n** and continue **-r** at VP/clause; on **e**, **-l** / **-m** = exception / replacement — not ordinary reference-suffix senses). Phrase-level is a **left fence** (coordinator first; optional shared modifiers immediately after; juxtaposed conjuncts; **no** `-r`). VP-level and clause-level are **left-open opener / `-r` continue** (opener first; `-r` before every non-initial conjunct). There are **no** bare (prefix-less) coordinator particles.
