...
c) Easy to tell the components of a compound word because **x** separates them. Mid-word **x** is only the compound joiner (never part of a root); word-initial **x** is the discourse-marker prefix. Closed [span fences](quotations.md) also use mid-word **`x`**: **open** `{PoS}{TYPE}xa{ENDING}` (TYPE **a** quote / **e** aside / **o** mention / **u** opaque; e.g. `daxal`); **close** **`xuxul`** (pop one) / **`xuxum`** (pop all). Parser cue: after PoS, a single TYPE vowel then `x` then **`a`** + ending → span open; exact `xuxul` / `xuxum` → span close; a longer root before `x` → ordinary compound (lexicon, [values](values.md), [ability](special-vocabulary.md#ability)).
...
g) **Span fences (quote / aside / mention / opaque):** [Opens](quotations.md) are ordinary PoS + TYPE vowel (**a** / **e** / **o** / **u**) + mid-word **`x`** + open **`a`** + ending (**-l** / **-m**). **Closes** are only **`xuxul`** / **`xuxum`** (not clause joins `xul` / `xum`, not left-bound `l`, not a global `xl` cluster). No stacked vowels on TYPE. Discourse-only opens use PoS `/x/` (`xaxal`, `xuxal`, …).