...
**Done when:** no learner-visible *clause force* / *question force* / *force word* for this job; `#clause-force` still resolves.
...
**Do:** arity-1 *focus* → **single-item**; arity-0 *bare* → **standalone**. Headings: *Starter forms (single-item and standalone)*; *Yes/no with single-item / standalone*. New ids `single-item-phrase`, `standalone-phrase` beside the old ones. Link text to `coordination.md#focus-phrase` may stay (silent id) or switch to the new id.
...
**Polar:** **yes/no** for the question type; **polar stance** for `jael` / …. Drop *reaction word* as the left-edge slot (order: vocative → polar stance → act word). Drop *polar question*, *yes/no polarity* as extra keys. Keep `#yes-no-polarity` as a silent id if the heading text changes.
...
| Roles | role compound | participant compound (anchor may stay) | `roles.md`, `x-compounds.md` |
...
| Value *topic* vs `ahaza` **as-for** | `values.md`, `core.md` `#adverb-topic-and-free-order-ties` (new id `as-for`; keep old id) |
...
2. Edit visible prose, headings, tables, See also. Add new anchors; keep old ids.
