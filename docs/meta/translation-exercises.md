...
5. **Spoiler answers** — VitePress `::: details Show answer` (or a clear custom label). The answer is the Agalan sentence or **loose** free English — not a second grammar lecture. No visible morph-gloss wall inside the spoiler unless the drill is explicitly about packaging. Every item needs a hidden morph line `<!-- gloss: z-Azawan | v-chair -->` immediately after the Agalan (spoiler for English → Agalan; prompt line for Agalan → English). That line is the [lexicon sense](glosses.md#no-lexicon-pos-specials) (`v-eye`, not `v-see`); `lint:agalan` compares it to the parser.
