...
| **Vocab** | The **Roots used here** table is this setting’s lexicon (names + content used in the items). Closed morphs (`darl`, **`yal`**, join vowels, …) appear in spoilers when taught; they need a table row only when the drill English is that word. |
...
**English pronouns match Agalan pronouns only.** A name word (`zazawan`, `duhubun`, vocative `yuhubun`) is *Azawan* / *Uhubun* in English — never *they* / *her*. Use *they* / *them* (Azawan), *he* / *him* (Ululon), or *she* / *her* (Uhubun) **only** when the Agalan form is a pronoun: letter or full-root **-r**, or the specials **`ugobo` / `edone` / `aha` / `enenu`**, and only on a checkpoint that is teaching those. Parentheticals follow the same rule (*calling Uhubun*, not *calling her*). Dummy *I* / *you* for untaught **`ugobo` / `edone`** stays forbidden; *I* for the **`SELF`** slot is the learner, not a dummy.
