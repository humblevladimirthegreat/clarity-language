...
7. **New content roots every checkpoint** — keep the shared bank for glue (house names, *walk* / *see* / *tell*, **`adoro`**). Each checkpoint’s table includes about **five content roots** that were **not** in the previous [sidebar](../grammar/.vitepress/config.ts) file’s drill table (house names do not count; skip pages with no checkpoint). Draw those extras from the [shared bank](drill-generation.md#root-bank), this stage’s worked examples, or recycle already taught on the path. Do **not** edit teaching examples just to seed drills. Do **not** mine [lexicon-published.csv](../../data/lexicon-published.csv) for variety on the fly. Recycle earlier extras on later sidebar files. New table rows exist because drills use them.
...
11. **Plausible scenes** — every prompt is something a person or animal could actually do. Do **not** write surreal props-as-agents (*a book sleeps*, *the blue house sings*). Humor is rivalry and composure, not cartoon physics. If the page’s stack is not a people scene (citation-only, number charts, span fences), keep items plausible for *that* decision; do not force a triangle beat.
12. **Mild comedy** — at least half the items have a slightly absurd *situation* (who glances, who charges, who is being performed for). Every item still tests **this** stage’s decision. First teach lines on the grammar page may stay plain; checkpoints may be weirder. Do not make the joke a morph trap (English homograph, untaught overlay). Do not mock appearance. Relax the half-comedy bar when house rivalry would not test the morph.
