...
- Mid-word **`x`** families: rule alternation order = doc decision order (closes → span open → role → greeting bid `/j/`+name+vowel+**-n` → value/ability → numeric → ordinary compound)
...
3. Else value/ability vs [greeting bid](../grammar/x-compounds.md#greeting-bid) (`/j/` + name + **-n**) vs published root
