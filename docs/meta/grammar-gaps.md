...
| G-I08 | *now* (speech time) | zero offset on a channel: `… theraram bohoram g+0 …`; otherwise the unmarked default | missing | knowing.md | **Deliberately not shipped as a word:** a short *now* would tempt use as a generic past/present in place of evidentials. Later: a verbose construction (*in this moment*) so the meaning is available but costs more than a channel. `h#e0` is not *now* | P1 |
...
| G-J16 | *ago* | channel + signed measure offset: `zazawan thuvuvum bohoram g-3 vawalal.` (*three hours ago*); `+` = *from now* | covered | knowing.md | Offset dates the event, never the moment of learning. No day / week / year unit yet (G-J29) | — |
...
| G-J29 | time units *day / week / month / year* | only `ohora` *hour* and `ozoba` *second* are published units | missing | numbers-applied.md | Lexicon: publish day / week / month / year unit roots so offsets read naturally (*three days ago*) | P1 |
