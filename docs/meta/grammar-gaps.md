...
| G-B11 | *still* (continuing state/action) | RESIDUE covers only a result that is *still on the tally* (`thonenom`). Ongoing *still walking* has no taught form | awkward | knowing.md | Teach continuation *still* separately, e.g. an `/h/` on published `ozodo` *cessation* with `u` negation, or a stance join Decided 2026-09-26: treat G-B11–B14 as one phasal set (**a** *already* / **o** *still* / **e** *not yet* / **u** *no longer*), endings = permanence (**-l** not expected to revert, **-m** could change; no **-r**). Spelling open: `x` + vowel on a host collides with ability (`hozodoxal`). | P1 |
| G-B12 | *already* | Not taught. `helabam barl` orders two events and does not say "sooner than expected" | missing | knowing.md | see G-B11 (phasal set) | P1 |
| G-B13 | *not yet* | Taught only for ability: `vuzunuxol` *can't yet* ([intention#incapability](../grammar/intention.md#incapability)). No general *hasn't walked yet* | awkward | intention.md / knowing.md | see G-B11 (phasal set) | P1 |
| G-B14 | *anymore* / *no longer* | FORMER `themebem` for habits only. A state that stopped (*doesn't live here anymore*) is not taught | awkward | knowing.md | see G-B11 (phasal set) | P1 |
...
| G-B33 | causative *let X do* (permission) | Permission **-l** with a hosted `/b/` grantor: `zazawan vawalal therenel bululon.` (*Ululon lets Azawan walk*) — [values#permission](../grammar/values.md#permission) | covered | values.md | — | — |
...
- Permission (G-B20, G-B33) filled 2026-09-26 in values.md#permission / #consent.
...
| G-C24 | quantity *enough (bread)* | tie against a need bar: `zadedal zegen zael g+.`; whose need = `/b/` after the scale ([comparatives#need-benchmarks](../grammar/comparatives.md#need-benchmarks)) | covered | comparatives.md | — | — |
| G-C25 | *too many / too much* (excess) | rank above a need bar (`zadedal zegen zel g+.`) or above *my standard* (`zuroron`) ([comparatives#need-benchmarks](../grammar/comparatives.md#need-benchmarks)) | covered | comparatives.md | — | — |
...
| G-D10 | *too* ADJ (excess: *too hot to drink*) | rank above a need bar on the quality: `zadedal zuhuhen zel gohodol.` ([comparatives#need-benchmarks](../grammar/comparatives.md#need-benchmarks)) | covered | comparatives.md | — | — |
| G-D11 | *enough* (*warm enough to swim*) | tie **`ae`** against a need bar on the quality ([comparatives#need-benchmarks](../grammar/comparatives.md#need-benchmarks)) | covered | comparatives.md | — | — |
...
| G-H03 | thanks (*thank you*, *thanks for the gift*) | a lone met `/th/` value is the thanks: `thonogotham.`; need root says what for, ending = appreciation channel — [values#thanks-sorry](../grammar/values.md#thanks-sorry) | covered | values.md | — | — |
| G-H04 | apology (*sorry*, *I apologize*) | unmet `/th/` value + `/b/` owner of the need: `thonogothum bedonen.`; changeability grades it (**-l** deep / **-m** default / **-r** *my bad*) — [values#thanks-sorry](../grammar/values.md#thanks-sorry) | covered | values.md | — | — |
