...
- **Existentials:** *there is / there is no* (G-F06–F07); verbless subject + `/ɡ/` is taught only as predication (property / kind), not existence.
...
| G-F06 | existential *there is / there are* (*There is a dog near the bank*) | none taught. Verbless subject + `/ɡ/` **is** taught, but only as predication (`zazawan gomonam.` *Azawan is challenging*, `zazawan godogol.` — [predication](../grammar/predication.md#beginner)). Parser also accepts `zodogol om banabal.` / `zodogol g+3 ol banabal.`, but no page says a bare subject (or subject + hook) asserts existence; `zodogol g+3` would read as predication (*the dogs are three*) under the taught rule; bare `/b/` in `thadorom banunul` is glossed *if there is rain* only incidentally ([causation](causation.md#beginner)) | missing | predication.md | Teach verbless subject-only clause (subject + optional count `/ɡ/` + hook place) as existential; note ambiguity with predicative `/ɡ/` count (`zodogol g+3` *there are three dogs* vs *the dogs are three*) and settle it | P1 |
| G-F07 | negative existential (*There is no dog here*) | set join `zodogol zul.` parses but is taught as *not a dog* ([joins#negation-u](../grammar/joins.md#negation-u)), not *there is no dog*; standalone join *there is no biggest* taught only for superlatives ([comparatives](comparatives.md)); `zul` alone = *no* ([joins#negation-u](joins.md#negation-u)) | missing | predication.md, joins.md | Fold into G-F06: `zodogol zul om banabal.` = *there is no dog near the bank* | P1 |
...
- Parser accepts verbless clauses freely (`zodogol.`, `zululon dagadal.`, `zodogol om banabal.`, `zodogol zul om banabal.`). Only subject + `/ɡ/` has a taught reading (predication); no page teaches the others as existential, gapping, or fragment readings; the docs need to pick one reading (G-F06 / G-F14 conflict).
...
| G-G20 | *at most N* (≤) | range from zero: `zagadalx g+0 g+5 gal gurulem vawalal.` | awkward | numbers-applied.md | Inclusive *≤* twin of the `oe` ray (only `≥` has a single-item form, `z+5 zoel`; rank `e` is strict `<`) | P1 |
