https://main.d2xds94zsgwptg.amplifyapp.com

# TODO
review TTS
consider removing emojis from docs (but keep in lexicon)
advanced section somewhere on how order affects emphasis
we currently allow skipping glosses in grammar files if the translation is trivial. We should require glosses everywhere, including trivial, so we can have stricter gloss count checking
local llm systematically review entire lexicon

## Evidentiality / tense

- Dedicated observation vs imagination **tense** (stacks with evidentiality channels; does not replace them). Live vs memory observation is **LIVE** (`adeze`) vs **WITNESSED** (`uvuvu`) — not a past/future slot.
- Consider sense words for observation

Dictionary entries (open lexicon / concepts — not the closed list above):
-habits: triggered
-unconditional self acceptance
-apology
-percentage chances for true
-Inner why?
-how inner self protects
-planning has importance value, consider numbered alternates
-pleasure now vs glad afterwards
-types of resistance: thoughts, behavior, attention

assertiveness rights: Ground yourself in the idea that you’re allowed to:
-say “no”
-express opinions
-make mistakes
-be treated with basic respect and be heard
-change your mind
-ask for what you need
-feel whatever emotions arise
-take time to decide
-make decisions without explaining yourself
-say “I don’t know” / “I don’t understand”
-end conversations

setback responses:
-not change yet
-not worth it, begin again
-accept
-this is training(difficult)
-be at peace anyway
-curiosity
-defusion
-distancing

