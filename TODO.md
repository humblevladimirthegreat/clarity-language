...
- NP-level coordination — **done:** `/z/` `/d/` `/b/` + same vowel series, **Oxford** style (not `/x/` **-r**), in `docs/coordination.md`
- AP-level coordination — **done:** `/ɡ/` + same vowel series, **Oxford** style, in `docs/coordination.md` (`gam`…; shared post-chain `/w/`)
- VP-level coordination — **done:** `/v/` + same vowel series, **opener / `-r`** (not Oxford); `/h/` clause-external over all VPs — `docs/coordination.md`
- Correlatives: *both…and, either…or* (word-, NP-, AP-, VP-, and clause-level *and* / *or* / *nor* are in `docs/coordination.md`)
...
Update the `/x/` section as this fuller class lands. (Documented elsewhere, not `/x/`: word-/NP-/AP-/VP-level *and*/*or* = `docs/coordination.md`; topic = `/h/`+`/b/`; focus = word order; *if/because/although…* = `/h/`+`/b/` next-clause pronoun.)
