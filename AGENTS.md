...
| `docs/numbers.md` | Numeral grammar, including [digitless / zero-group forms](docs/numbers.md#zero-digit-groups) (`g+l` = plural count `>1`, …), [digitless exponents](docs/numbers.md#digitless-exponents) (`g+el` = +∞, `g+1el` = *one gazillion* hyperbole, `g#1el` = *the gazillionth* / *umpteenth*, `g#2e-l` = *2nd from the end*, `g#el` = last place, `g#e-l` = start/beginning place, **`x#e-l`** = discourse *Starting with*, **`x#el`** = *Finally*, …; `/v/` `/h/` without-bound overlays), [percent denominators](docs/numbers.md#percent-denominators) (whole NP + `/ɡ/` `…jo`), [ranges](docs/numbers.md#ranges) (SHARED continuum `/ɡ/` + two number endpoints; bare `zel z+3l z+5l` = preference) and [measure phrases](docs/numbers.md#measure-phrases) (linked from the language reference). |
...
| `docs/quotations.md` | Span fences (writing PoS + optional **`@`** / **`~`** after PoS + `[…]` / `{…}` / `(…)` / `<…>` (`d@[…]` proper, `d~[…]` paraphrase; hedged `@~` spelled `d@[…]`); open `{PoS}{TYPE}x{EDGE}{ENDING}` — e.g. `daxal` / `daxol` / `daxan` / `daxar`; TYPE **a** cite / **e** aside / **o** mention / **u** opaque; EDGE **a** multi / **e** clause-scoped / **o** atomic / **u** empty; **-l** exact / **-m** paraphrase / **-n** proper / **-r** span anaphor; close **`xuxul`** / **`xuxum`**; linked from the language reference).
...
| `docs/coordination.md` | **Joins** — phrase-, VP-, clause-level **fence**; **set joins** vs **rank joins**; arity **bare** / **focus** / **list**; **-r** member; clause soft **-n** (`xan` = *and then*); VP `-n` = [join-act verbs](docs/special-vocabulary.md#join-act-verbs); `/ɡ/` `/h/` `-n` = [join-relations](docs/special-vocabulary.md#join-relations); linked from the language reference. |
| `docs/revisers.md` | Prefix-less **revisers** (**al** / **am** / **an** / **el** / … — in-clause *including* / *rather* / *instead* / *except*; before `/j/` = discourse *additionally* / *in other words* / *instead* / *except*; *Starting with* = **`x#e-l`**, *Finally* = **`x#el`**); linked from the language reference. |
...
| `docs/special-vocabulary.md` | Special morphology / closed roots (not needs, not restrictors): **join-act verbs** (`van` / `von` / …); **join-relations** (`gan` / `han` / … — unary `/b/`); **role compounds** (**`a`/`u`/`o` x ROOT** = agent / patient / reltum — no *attacker* lexicon); **ability** / **incapability** — primary **host + `xu`/`xa`** + changeability endings (`v-singxun`); fallback closed **`ABIL`** when hostless; **plan / predict**, **emotion compose** (ACT + LOCUS + reused values), and **COMMENT** (mindfulness noting — six synonymous `/h/` exponents) = closed `/h/` mood roots (**not** `x`-compounds); linked from the language reference. |
| `docs/x-compounds.md` | Mid-word **`x`** **parser families** (span vs role vs values/ability vs ordinary compound) — linked from the language reference / phonology. |
