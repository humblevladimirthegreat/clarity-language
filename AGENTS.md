...
| `docs/special-vocabulary.md` | Special morphology / closed roots (not needs, not restrictors): **ability** / **incapability** — primary **host + `xu`/`xa`** + changeability endings (`v-singxun`); fallback closed **`ABIL`** when hostless; linked from the language reference. |
