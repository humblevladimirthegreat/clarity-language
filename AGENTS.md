...
| `docs/plurality.md` | Plural grammar (**-sh** = group referent on nouns / event-sets / vocatives; **collective** on `/ɡ/`; unused on `/w/` `/h/` `/x/` and number words; generics / *every K* via coordination, not **-sh** — [universals](docs/coordination.md#universals-domains-generics); linked from the language reference). |
