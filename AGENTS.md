...
| `docs/quotations.md` | Span fences (writing PoS + `[…]` / `{…}` / `(…)` / `|…|` + **`~`** = paraphrase; open `{PoS}{TYPE}xa{ENDING}` — e.g. `daxal`; TYPE **a** quote / **e** aside / **o** mention / **u** opaque; close **`xuxul`** / **`xuxum`**; **-l** exact / **-m** paraphrase on opens; linked from the language reference).
