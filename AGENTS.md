...
| `docs/coordination.md` | **Joins** — phrase-, VP-, clause-level **fence**; **set joins** vs **rank joins**; arity **bare** / **focus** / **list**; **-r** member; clause soft **-n** (`xan` = *and then*); VP `-n` = [join-act verbs](docs/special-vocabulary.md#join-act-verbs); linked from the language reference. |
...
| `docs/special-vocabulary.md` | Special morphology / closed roots (not needs, not restrictors): **join-act verbs** (`van` / `von` / … — *include* / *choose as the one* / …; not soft joins); **ability** / **incapability** — primary **host + `xu`/`xa`** + changeability endings (`v-singxun`); fallback closed **`ABIL`** when hostless; **plan / predict**, **emotion compose** (ACT + LOCUS + reused values), and **COMMENT** (mindfulness noting — six synonymous `/h/` exponents) = closed `/h/` mood roots (**not** `x`-compounds); linked from the language reference. |
