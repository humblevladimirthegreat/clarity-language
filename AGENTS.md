...
| `docs/coordination.md` | Word-level Oxford (**al**, … before last); phrase-level final fence (**zam**, **gam**, … after last conjunct); shared opener/`-r` for VP (`/v/`: **vam** / **var**) and clause (`/x/`); **-l**/**-m**/**-n** = closed/open/named or soft, or exception/replacement on **e**; linked from the language reference. |
