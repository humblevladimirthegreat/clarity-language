...
| `docs/special-vocabulary.md` | Special morphology / closed roots (not needs, not restrictors): **ability** / **incapability** — primary **host + `xu`/`xa`** + changeability endings (`v-singxun`); fallback closed **`ABIL`** when hostless; **plan / predict**, **emotion compose** (ACT + LOCUS + reused values), and **COMMENT** (mindfulness noting — six synonymous `/h/` exponents) = closed `/h/` mood roots (**not** `x`-compounds); linked from the language reference. |
